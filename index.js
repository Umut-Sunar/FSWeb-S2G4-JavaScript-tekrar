...
const  CemberinCevresi = (yaricap) =>  {return 2*pi*yaricap }
...
/* 	GÖREV 3:
  ...
  Bu dizi içinde 0 ile 1000 arasında rasgele oluşturulmuş tam sayılar ve ondalıklı sayılar bulunmaktadır. Bu diziyi kullanarak aşağıdakileri uygulayın:
		3a. enbuyuk ve enkucuk i  sminde 2 adet değişken tanımlayın ve sayilar dizisindeki en küçük sayı ile endeğişkenlere atayın. (for döngüsü kullanın)
      büyük sayıyı bu 
...
// 3a çözümü
  ...
  for (let i= 0 ; i<sayilar.length;i++) {
    ...
    if(enkucuk>sayilar[i] & i<sayilar.length) {
      enkucuk = sayilar[i]
      
    } 

    if ( ( enbuyuk<sayilar[i+1]) && (i<sayilar.length)) {
        enbuyuk = sayilar[i+1]
...
ucebolunenlerintoplami=ucetambolunenler.reduce(  (num,currentValue) =>  {
